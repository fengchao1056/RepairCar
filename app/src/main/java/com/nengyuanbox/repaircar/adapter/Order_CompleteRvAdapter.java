...
//订单已完成
public class Order_CompleteRvAdapter extends RecyclerView.Adapter<Order_CompleteRvAdapter.ViewHolder> {


    private List<CompleteOrderBean.DataBeanX.DataBean> list = new ArrayList<>();
    private Context context;

    public Order_CompleteRvAdapter(List<CompleteOrderBean.DataBeanX.DataBean> list, Context context) {
        this.list = list;
...
//           期望時間
        ...
        if (!RxDataTool.isEmpty(list.get(position).getApply_user_evaluate())){
            holder.ll_repair_star.setVisibility(View.VISIBLE);
            holder.tv_repair_star.setText(list.get(position).getApply_user_evaluate());
