...
//    public void startRoutePlan(int flag ,String  myAreax,String  myAreay,String myaddress, String  orderAreax,String  orderAreay,String orderAddress) {
    public void startRoutePlan(int flag , String  orderAreay,String  orderAreax,String orderAddress) {
        switch (flag) {
            case FLAG_BAIDU:
                goToBaiduMap("", "", "", orderAreax, orderAreay, orderAddress);
                break;
            case FLAG_GAODE:
                goToAutonaviMap("39.987045", "116.50489", "", orderAreax, orderAreay, orderAddress);
                break;
            case FLAG_TENCENT:
                geToTencentMap("39.987045", "116.50489", "", orderAreax, orderAreay, orderAddress);
                break;
            case FLAG_GOOGLE:
                break;
        }
    ...
    public void startRoutePlan(int flag ,String  myAreay,String  myAreax,String myaddress, String  orderAreay,String  orderAreax,String orderAddress) {

        ViseLog.d("FC"+myAreay+ "----"+myAreax+"---"+myaddress);
        switch (flag) {
            case FLAG_BAIDU:
                goToBaiduMap(myAreax, myAreay, myaddress, orderAreax, orderAreay, orderAddress);
                break;
            case FLAG_GAODE:
                goToAutonaviMap(myAreax, myAreay, myaddress, orderAreax, orderAreay, orderAddress);
                break;
            case FLAG_TENCENT:
                geToTencentMap(myAreax, myAreay, myaddress, orderAreax, orderAreay, orderAddress);
                break;
            case FLAG_GOOGLE:
                break;
        }
    ...
    private void goToAutonaviMap(String slat, String slon, String sname, String dlat, String dlon, String dname) {
        try {
            String uri = "amapuri://route/plan/?slat=" + "" + "&slon=" + "" + "&sname=" + "" + "&dlat=" + dlat + "&dlon=" + dlon + "&dname=" + dname + "&dev=0&t=0";
            Intent intent = new Intent();
            intent.setData(Uri.parse(uri));
            intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            intent.setPackage(RoutePlanUtil.PACKAGE_GAO_DE);
            CommApplication.getContext().startActivity(intent);
        } catch (Exception e) {
    ...
    private void geToTencentMap(String slat, String slon, String sname, String dlat, String dlon, String dname) {  //这里 from 和 to 不能不写或者空白符
        try {
            String tencnetUri = "qqmap://map/routeplan?type=drive&from=" + "" + "&fromcoord=" + "" + "," + "" + "&to=" + dname + "&tocoord=" + dlat + "," + dlon + "&policy=0&referer=" + CommApplication.getContext().getResources().getString(R.string.app_name);
            Intent intent = new Intent();
    ...
    public static boolean isInstalledBaidu() {
        ...
        ResolveInfo resolveInfo = CommApplication.getContext().getPackageManager().resolveActivity(intent, 0);
    ...
    public static boolean isAvilible(String packageName) {
        //获取packagemanager
        final PackageManager packageManager = CommApplication.getContext().getPackageManager();
        //获取所有已安装程序的包信息
